...
public:
	...
	{
		msg_.set_username(userName);//请求包含用户名
	};

	const std::string& get_userName() { return msg_.username(); };
	virtual std::ostream& dump(std::ostream& out) const;
	virtual i32 Bytesize() { return msg_.ByteSize(); };//消息大小
	virtual bool SerializeToArray(char* buf, int len) { return msg_.SerializeToArray(buf, len); };//反序列化
