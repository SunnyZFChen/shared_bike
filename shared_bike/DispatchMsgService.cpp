...
pthread_cond_t DispatchMsgService::queue_cond = PTHREAD_COND_INITIALIZER;
...
BOOL DispatchMsgService::open()
{
	...
	pthread_mutex_init(&queue_mutext, nullptr);
	pthread_cond_init(&queue_cond, nullptr);
	//LOG_DEBUG("线程池初始化......");

	//printf("线程池初始化......\n");
	/*tp = thread_pool_init();
	response_tp = thread_pool_init(); */ // 响应事件处理线程池 <== 新增
...
void DispatchMsgService::close()
{
	...
	pthread_mutex_lock(&queue_mutext);
	svr_exit_ = true;
	pthread_cond_broadcast(&queue_cond);  // 唤醒所有等待线程
	pthread_mutex_unlock(&queue_mutext);

	thread_pool_destroy(tp);
	thread_pool_destroy(response_tp); // 新增
	pthread_mutex_destroy(&queue_mutext);
...
void DispatchMsgService::handleAllResponseEvent(NetworkInterface* interface)
{
	...
	pthread_mutex_lock(&queue_mutext);
	// 等待条件变量
	//while (response_events.empty()) {
	//	pthread_cond_wait(&queue_cond, &queue_mutext);  
	//}
	while (!response_events.empty()) {
		iEvent* ev = response_events.front();
		response_events.pop();

		pthread_mutex_unlock(&queue_mutext);

		// 将响应任务提交给响应线程池
		thread_task_t* task = thread_task_alloc(sizeof(ResponseContext));
		ResponseContext* ctx = (ResponseContext*)task->ctx;
		ctx->ev = ev;
		ctx->interface = interface;

		task->handler = DispatchMsgService::handle_response_task;
		thread_task_post(response_tp, task);
...
void DispatchMsgService::handle_response_task(void* data) {

	ResponseContext* ctx = (ResponseContext*)data;
	iEvent* ev = ctx->ev;
	NetworkInterface* interface = ctx->interface;
	...
	case EEVENTID_ADDBIKE_RSP:
	case EEVENTID_DELETEBIKE_RSP:
		DispatchMsgService::getInstance()->sendPesponseMessage(ev, static_cast<EventID>(EventId), interface);
		break;
	case EEVNETID_EXIT_RSP:
	{
		ConnectSession* cs = (ConnectSession*)ev->get_args();
